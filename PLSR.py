...
def GuassianKernel(x, y, sig = 1.0):
    """ Calculate the Gaussian Kernel
    """
    return np.exp(-np.sum((x-y)**2) / (2.0 * sig**2)) * 0.3989422804014327 / sig
...
class KernelPLSR(BR.BasicRegression):
    ...
    def fit(self, X, Y, d = None, Kernel = 'Gaussian', max_iter = 500, eps = 1e-6):
